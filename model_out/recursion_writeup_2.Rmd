...
For US kids, we found that the inferred probability of making an error correlated with their memory performance $(\rho=-0.43; p=0.02)$. Supplementary figure X shows this... [where should this go?].
...
The inferred probabilities that individual participants were using each strategy, represented by the parameter $\theta_g$, is shown below.
...
m.alpha <- m.alpha %>%
        ...
        mutate(who=as.factor(gsub("kids","US Kids",as.character(who)))) %>%
            group_by(who) %>%
           mutate(mean_val=mean(value)) %>%
            mutate(upper=quantile(value,0.9)) %>%
            mutate(lower=quantile(value,0.1)) %>%
            top_n(n=1,wt=sample)
