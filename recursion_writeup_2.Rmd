...
file_beta <- "merged_data/merged_beta.csv"
data_beta = read.csv(file_beta)
data_beta$ID <- seq.int(nrow(data_beta))


file_theta <- "merged_data/merged_theta.csv"
...
p.thetabeta <- ggplot() +
    ...
    stat_summary(data=data_theta_x,
                   fun.y=median, fun.ymin=median, fun.ymax=median, aes(x=as.numeric(who), y=value, group=part, shape=who, color="Individual"), geom="point",
                      alpha=0.75, size=2.5, position=position_dodge(width=0.45)) +
    stat_summary(data=data_beta_x, 
                 fun.data=fn_summary_thetabeta, aes(x=as.numeric(who), y=value, shape=who, color="Group"),alpha=0.5, size=1.25) +

    stat_summary(data=data_beta_x, 
                  fun.data=fn_summary_thetabeta, aes(x=as.numeric(who), y=value, color="Group"), 
                 geom="errorbar", alpha=0.9, size=0.5, width=0.2) +
...
#p.thetabeta
#ggsave("figs/fig5_2.png", width=8,height=5, dpi=400)
#ggsave("figs/fig5_2.pdf", width=8,height=5)
```



```{r, echo=FALSE, warning=FALSE, message=FALSE}
file_noise <- "merged_data/merged_noise.csv"
...
ce.counterfactual <- data_theta_y %>%
                    group_by(who, part) %>%
                      ...
                      mutate(ce = 0.5 * value * (which == "OOCC") +  value * (which == "OOMM")) %>%
...
p.noise.counterfactual <- ggplot(data=ce.both, aes(x=who, y=ce, group=noise)) +
...
ggsave("figs/noise_counterfactual.pdf", width=4.5,height=4)
#p.kiddig 
#reg.kiddig

p.err
ggsave("figs/noise.pdf",width=4.5,height=4)
...
n_kid <- nrow(subset(data_theta_mean_x, grepl("kid", data_theta_mean_x$who)))
n_tsim <- nrow(subset(data_theta_mean_x, grepl("Tsimane", data_theta_mean_x$who)))
...
(n_kid_worse + n_tsim_worse)/ (n_kid+n_tsim)
...
img1 <-  rasterGrob(as.raster(readPNG("figs/lda_graph.png")), interpolate = FALSE)
x <- grid.arrange(img1, p.thetabeta, ncol=2, top="Figure 5")

#p.thetabeta
#ggsave("figs/fig5_2.png", width=9,height=5)
#ggsave("figs/fig5_2.pdf", width=9,height=5)
...
ce.counterfactual.av <- ce.counterfactual %>%
                        group_by(who) %>%
                          mutate(ce_low = quantile(ce,0.05)) %>%
                          mutate(ce_high = quantile(ce,0.95)) %>%
                        mutate(ce = mean(ce)) %>%
...
ggplot(data=data_beta_name, aes(x=which, y=mean_val, fill=who)) +
      geom_point(position=position_dodge(width=0.93),size=1) +
      ...
      geom_hline(linetype="dashed", yintercept=0.08) +
      theme(legend.position=c(0.15,0.8)) +
      ylim(0,1.0) + scale_fill_discrete(labels=c("Monkeys", "US kids", "Tsimane' adults", "US adults"))


ggsave("figs/all_beta.pdf",width=8,height=4)
```
...
ggsave("figs/all_theta.pdf", width=6,height=6)
...
ggsave("figs/kid_error.pdf", width=5,height=4)
```
...
m.alpha <- read.csv("merged_data/merged_alpha.csv")
...
ggsave("figs/alpha.pdf", width=5,height=4)
...
monkey_beta_other <- read.csv("other_monkey_merged/merged_beta.csv")
monkey_theta_other <- read.csv("other_monkey_merged/merged_theta.csv")

monkey_noise_other <- read.csv("other_monkey_merged/merged_noise.csv")

monkey_beta_other <- monkey_beta_other %>%
                    group_by(who, which) %>%
                    mutate(value_high = quantile(value, 0.95)) %>%
                    mutate(value_low = quantile(value, 0.05)) %>%
...
monkey_noise_other <- monkey_noise_other %>%
                    group_by(who, part) %>%
                    mutate(value_high = quantile(value, 0.95)) %>%
                    mutate(value_low = quantile(value, 0.05)) %>%
                    ...
                    mutate(value = mean(value)) %>%
                    top_n(n=1,wt=sample) %>%
                    mutate(p_ce = (1-(1-value)**4)) %>%
                    mutate(p_ce_high = (1-(1-value_high)**4)) %>%
                    mutate(p_ce_low = (1-(1-value_low)**4)) 




monkey_theta_other <- monkey_theta_other  %>%
                    group_by(who, part, which) %>%
                    mutate(value_high = quantile(value, 0.95)) %>%
                    mutate(value_low = quantile(value, 0.05)) %>%
...
```{r, fig.width=8,fig.height=4}
p.monkey1 <- ggplot(data=subset(monkey_beta_other, grepl("monkeys1", monkey_beta_other$who)), aes(x=which, y=value)) +
            geom_point() +

          geom_col(color="black", fill="gray", width=1.0,size=0.5) +
          geom_errorbar(aes(ymin=value_low, ymax=value_high), width=0.2, alpha=0.6) +
          paper_theme +
        geom_hline(yintercept=1/12., alpha=0.5, linetype="dashed") +
            ylab(expression(paste(beta, " value"))) +
          ylim(0,0.6)


p.monkey3 <- ggplot(data=subset(monkey_beta_other, grepl("monkeys3", monkey_beta_other$who)), aes(x=which, y=value)) +
            geom_point() +
...
ggsave("figs/s6.pdf", width=8,height=4)

p.monkey3
ggsave("figs/s7.pdf",width=8,height=4)

monkey_beta_other <- monkey_beta_other %>%
                ungroup %>%
                mutate(who2=who) %>%
                mutate(who2 = gsub("monkeys1","1",who2)) %>%
                mutate(who2 = gsub("monkeys3","3",who2)) %>%
                mutate(who2 = gsub("monkeys","2",who2))

monkey_beta_other$who2 <- factor(monkey_beta_other$who2)
...
monkey_theta_other$who2 <- factor(monkey_theta_other$who2)


p.m1 <- ggplot(data=subset(monkey_beta_other, grepl("OO[MC][MC]",monkey_beta_other$which)), 
       aes(x=who, y=value, color=which)) +
        geom_point()

p.m2 <- ggplot() +
        geom_col(data=subset(monkey_beta_other, grepl("OOMM",monkey_beta_other$which)), 
       aes(x=who, y=value, fill="Group", color="Group")) +
        geom_point(data=subset(monkey_theta_other, grepl("OOMM",monkey_theta_other$which)), 
       aes(x=who2, y=value, fill="Individual", color="Individual")) +
      scale_color_manual(values=c("#b30000","black"), labels=my.labs) +
      scale_fill_manual(values=c("#b30000","black"), labels=my.labs)+   guides(color=FALSE) +
      paper_theme


subs <- monkey_beta_other %>%
        filter(grepl("OO[MC][MC]", which) | grepl("OMOM", which))
